/* 
 *  DHT11 sensor example with ESP8266-01:
 *  
 *  1. Publish message every sensorDelayMS to "/sensor/temp" and "/sensor/humid" for temperature and humidity.
 *  2. Connect DHT11 data out to GPIO0.
 *  3. This example use Adafruit unified sensor library for DHT11 reading, hence you must install it first.
 *  
 */
...
#define WIFI_DEFAULT_SSID "SmartDHT11"
...
void initPublish() {
  if (client.connected()) {    
    ...
    client.publish(MQTT::Publish(obj.constructTopic("$name"), "Smart DHT11").set_retain(false).set_qos(1));
    client.publish(MQTT::Publish(obj.constructTopic("$model"), "SAM-DHT11").set_retain(false).set_qos(1));
    ...
    client.publish(MQTT::Publish(obj.constructTopic("$fw/name"), "DHT11").set_retain(false).set_qos(1));
